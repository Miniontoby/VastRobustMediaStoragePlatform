...
<form onsubmit={submitCallback} method="post">
	<input type="file" id="video" name="video" bind:files={files} accept="video/*" />
	<br />
				...
				<div class="p-4 md:p-5 text-center">
					{#if progress === 100}
						...
						<div class="mt-2">
							...
							<p>
								<a href={resolve('/(uploader)/videos/[id]', { id: uploadId })}>{uploadId}</a>
							</p>
						</div>
					...
					{:else}
						{#if step === 'processing'}
							<h3 id="dialog-title" class="mb-6">
								{m['pages.upload.processing.title']({ eta: etaString })}
							</h3>
						{:else}
							<h3 id="dialog-title" class="mb-6">
								{m['pages.upload.uploading.title']()}
							</h3>
						{/if}
						<div class="mt-2">
							{#if step === 'processing'}
								<p class="text-sm text-gray-400">
									{m['pages.upload.processing.description']({ progress: String(progress) })}
								</p>
							{:else}
								<p class="text-sm text-gray-400">
									{m['pages.upload.uploading.description']({ progress: String(progress) })}
								</p>
							{/if}
						</div>
						<div class="flex items-center justify-center space-x-4">
							<button type="button" class="btn btn-danger" onclick={cancelUpload}>
								{m['actions.cancel']()}
							</button>
						</div>
					{/if}
